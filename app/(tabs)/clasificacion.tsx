...
const OPCIONES_APUESTA = [
    ...
    },
    {
        ...
        parametros: [
            { tipo: "numero", label: "Victorias seguidas", key: "victorias" }
        ]
    },
    {
        id: "mas_coronas",
        titulo: "Conseguirá más coronas esta semana",
        // Esta no tiene parámetros, es directa
    }
];
...
export default function Clasificacion(): JSX.Element {
    ...
    function toggleSeleccion(miembro: Miembro) {
        ...
        if (existe) {
            setSeleccionados(prev => prev.filter(m => m.id !== miembro.id));
        } else {
            setSeleccionados(prev => [...prev, miembro]);
        }
    }
    ...
    async function confirmarSalida() {
        try {
            if (!ligaObj?.id || !user?.id) {
                alert("Datos de usuario o liga no disponibles");
                return;
            }
    ...
    function confirmarApuesta() {
        ...
        alert(`¡Apuesta enviada a ${miembroApuesta.nombre} por ${cantidadPuntos} puntos!`);
    ...
    if (!ligaObj) return <View style={styles.center}><Text style={styles.errorText}>Error: no se recibió liga.</Text></View>;
        ...
        <View style={styles.screen}>
            <View style={styles.headerRow}>
                ...
                <View style={{ flexDirection: "row", gap: 12 }}>
                    ...
                    <TouchableOpacity
                        style={[styles.headerBtn, { backgroundColor: "#D32F2F" }]}
                        onPress={salirDeLiga}
                    >
            ...
            {loading ? (
                <View style={styles.loadingRow}><ActivityIndicator size="large" color="#fff" /></View>
            ) : error ? (
                <View style={styles.center}><Text style={styles.errorText}>{error}</Text></View>
            ) : (
                <ScrollView contentContainerStyle={styles.listContainer}>
                    {miembros.map((m, idx) => {
                        const esYo = m.nombre === user?.nombre;
                        ...
                        return (
                            ...
                            >
                                <View style={styles.left}>
                                    <View style={styles.positionCircle}><Text style={styles.positionText}>{idx + 1}</Text></View>
                                ...
                                <View style={styles.actions}>
                                    {!modoSeleccion && !esYo && (
                                        <>
                                            ...
                                            <TouchableOpacity style={styles.actionBtn} onPress={() => alert(`Retando a ${m.nombre}`)}>
                                                <Ionicons name="flash" size={18} color="#FFC107" />
                                            </TouchableOpacity>
                                        </>
                                    )}
                                    {modoSeleccion && seleccionado && <Ionicons name="checkmark-circle" size={24} color="#FFD700" />}
                                </View>
                            </TouchableOpacity>
                        );
                    })}
            ...
            <Modal visible={modalApuestaVisible} transparent={true} animationType="fade" onRequestClose={() => setModalApuestaVisible(false)}>
                ...
                <TouchableOpacity
                    ...
                    onPress={() =>
                        router.push({
                            ...
                            params: {
                                usuarios: seleccionados.map(u => u.nombre).join(",")
                            }
                        })
                    }
                >
                    <Text style={styles.compareText}>
                        Comparar {seleccionados.length} jugadores
                    </Text>
                </TouchableOpacity>
            )}
...
const styles = StyleSheet.create({
    ...
    chip: { backgroundColor: "#333", paddingVertical: 8, paddingHorizontal: 12, borderRadius: 20, marginRight: 8, borderWidth: 1, borderColor: "#444" },
    ...
    btnDisabled: { backgroundColor: "#2e5c30", opacity: 0.5 },
