...
export default function Registre() {
    ...
    const containerStyle: ViewStyle = Platform.OS === "web"
        ? { width: "33%", minWidth: 350, alignSelf: "center" }
        : { width: "100%" };
    ...
    function registrar() {
        fetch('http://localhost:3000/registre', {
            ...
            body: JSON.stringify({ Usuari, Correu, Contrasenya }),
        })
            .then((response) => response.json())
            .then((data) => {
                console.log(data);
                if (data.error) { alert(data.error) }
                else { alert('Correu enviat correctamente'); router.push('/inici_sessio'); }
            })
    }
    ...
    function registrarConId() {
        fetch('http://localhost:3000/registrarConId', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ Usuari, Correu, Idsupercell, Contrasenya }),
        })
            .then((response) => response.json())
            .then((data) => {
                console.log(data);
                        ...
                        <TouchableOpacity
                            ...
                            onPress={() => {
                                ...
                                if (Usuari && Correu && Contrasenya && CContrasenya && Idsupercell) {
                                    if (Contrasenya === CContrasenya) { registrarConId(); }
                                    else { alert('Contrasenyes no coincideixen'); }
                                } else if (Usuari && Correu && Contrasenya && CContrasenya) {
                                    if (Contrasenya === CContrasenya) { registrar(); }
                                    else { alert('Contrasenyes no coincideixen'); }
                                } else { alert('Camps buits'); }
                            }}>
                            <Text style={{color: 'white', fontWeight: 'bold', fontSize: 20}}>Registrar-se</Text>
                        </TouchableOpacity>
                    </View>
                </View>
            </ScrollView>
